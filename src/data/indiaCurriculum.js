...
export function getBoardClasses(boardShortName) {
  return BOARD_CLASS_RANGES[boardShortName] || range(1, 12)
}
...
export function getSubjects(boardShortName, classNum) {
  ...
  if (boardShortName === 'ICSE') {
    ...
    if (n <= 5) return ['English', 'Mathematics', 'Environmental Studies', 'Hindi', 'General Knowledge', 'Computer Applications']
    if (n <= 8) return ['English', 'Mathematics', 'Science', 'History & Civics', 'Geography', 'Hindi', 'Computer Applications']
    return ['English Literature', 'English Language', 'Mathematics', 'Physics', 'Chemistry', 'Biology', 'History & Civics', 'Geography', 'Computer Applications', 'Hindi']
  }

  if (boardShortName === 'ISC') {
  ...
  if (['CBSE', 'NIOS', 'DoE', 'IB'].includes(boardShortName)) {
    if (n <= 2) return ['English', 'Mathematics', 'Environmental Studies', 'Hindi', 'General Knowledge']
...
export const OFFICIAL_PDF_URLS = {
  ...
  'class7_science': 'https://ncert.nic.in/textbook.php?gesc1=',    // Rationalized Science
  'class8_science': 'https://ncert.nic.in/textbook.php?hesc1=',
  ...
  'class7_mathematics': 'https://ncert.nic.in/textbook.php?gemh1=',    // Rationalized Maths
  'class8_mathematics': 'https://ncert.nic.in/textbook.php?hemh1=',
...
// Neither is on DIKSHA; chapters come from curated CISCE mappings first.
export function isICSE(boardShortName) {
  return boardShortName === 'ICSE' || boardShortName === 'ISC'
}

// ── Medium (language of instruction) ─────────────────────────────────────────
// State boards and NIOS typically offer multiple mediums.
// National boards (CBSE, ICSE, ISC, IB) are considered English-medium by default.
export const BOARD_MEDIUMS = {
  BSEAP: ['English', 'Telugu'],
  SEBA: ['English', 'Assamese', 'Bengali', 'Bodo'],
  AHSEC: ['English', 'Assamese'],
  BSEB: ['English', 'Hindi', 'Urdu'],
  CGBSE: ['English', 'Hindi'],
  GBSHSE: ['English', 'Marathi', 'Konkani'],
  GSEB: ['English', 'Gujarati'],
  HBSE: ['English', 'Hindi'],
  HPBOSE: ['English', 'Hindi'],
  JAC: ['English', 'Hindi', 'Urdu'],
  KSEEB: ['English', 'Kannada'],
  PUC: ['English', 'Kannada'],
  KBPE: ['English', 'Malayalam'],
  MPBSE: ['English', 'Hindi', 'Urdu'],
  MSBSHSE: ['English', 'Marathi', 'Urdu', 'Semi-English'],
  BSEM: ['English', 'Meitei (Manipuri)'],
  MBOSE: ['English', 'Khasi', 'Garo'],
  MBSE: ['English', 'Mizo'],
  NBSE: ['English', 'Nagamese'],
  BSE: ['English', 'Odia'],
  CHSE: ['English', 'Odia'],
  PSEB: ['English', 'Punjabi', 'Hindi'],
  RBSE: ['English', 'Hindi'],
  BSSS: ['English', 'Nepali'],
  TNBSE: ['English', 'Tamil'],
  BSETS: ['English', 'Telugu'],
  TBSE: ['English', 'Bengali'],
  UPMSP: ['English', 'Hindi', 'Urdu'],
  UBSE: ['English', 'Hindi'],
  WBBSE: ['English', 'Bengali', 'Hindi', 'Urdu'],
  WBCHSE: ['English', 'Bengali'],
  JKBOSE: ['English', 'Urdu', 'Hindi'],
  PBSE: ['English', 'Tamil'],
  DoE: ['English', 'Hindi', 'Urdu'],
  NIOS: ['English', 'Hindi', 'Urdu'],
}

/** Returns array of mediums for a board, or null if no medium dropdown is needed. */
export function getBoardMediums(boardShortName) {
  return BOARD_MEDIUMS[boardShortName] || null
}

// ── Stream (Science / Commerce / Arts) ───────────────────────────────────────
// Streams apply only to Classes 11–12 for most boards.
// ICSE ends at Class 10 — no stream needed.
export const STREAMS = ['Science', 'Commerce', 'Arts / Humanities']

/** Returns true when a board+class combination should show the Stream dropdown. */
export function boardNeedsStream(boardShortName, classNum) {
  if (parseInt(classNum) < 11) return false
  const noStreamBoards = new Set(['ICSE']) // ICSE only covers Classes 1-10
  return !noStreamBoards.has(boardShortName)
}

// Curated subject lists by stream for Classes 11-12
const STREAM_SUBJECTS = {
  Science: ['English', 'Mathematics', 'Physics', 'Chemistry', 'Biology', 'Computer Science', 'Physical Education'],
  Commerce: ['English', 'Mathematics', 'Accountancy', 'Business Studies', 'Economics', 'Computer Science'],
  'Arts / Humanities': ['English', 'History', 'Geography', 'Political Science', 'Economics', 'Sociology', 'Psychology', 'Hindi', 'Computer Science'],
}

/**
 * Returns subjects for a board/class, optionally filtered by stream.
 * Falls back to getSubjects() when stream is not applicable or not provided.
 */
export function getSubjectsForStream(boardShortName, classNum, stream) {
  if (!stream || parseInt(classNum) < 11) return getSubjects(boardShortName, classNum)
  return STREAM_SUBJECTS[stream] || getSubjects(boardShortName, classNum)
}
