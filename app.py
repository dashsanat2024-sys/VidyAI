...
def _build_ocr_prompt(exam_questions: list) -> str:
    """
    Visual-reasoning prompt: forces GPT-4o to spatially locate each question
    and detect the student's answer regardless of HOW it was marked
    (written, circled, ticked, written beside options, etc.).
    """
    ...
    question_ref = ("\nQuestion reference (use to LOCATE each question on the page):\n"
    ...
    def _process_one_page(args):
        ...
        try:
            ...
            if blank_pages and page_idx < len(blank_pages):
                ...
                # Fix 2: preprocess scanned image to enhance handwriting visibility
                img_preprocessed = _preprocess_image_for_ocr(img)

                blank_arr   = _np.array(blank_img.convert("RGB")).astype(_np.int16)
                scanned_arr = _np.array(img_preprocessed.convert("RGB")).astype(_np.int16)
                ...
                for q_idx, q in enumerate(page_qs):
                    ...
                    # Fix 2: apply full preprocessing pipeline to crop
                    crop_img = _preprocess_image_for_ocr(crop_img)

                    buf = _io.BytesIO()
                    crop_img.save(buf, format="PNG")
                    crop_b64 = base64.b64encode(buf.getvalue()).decode()

                    # Fix 1: visual-reasoning prompt for crop (handles circles/ticks)
                    if qtype == "objective":
                        ocr_p = (
                            f"This image shows ONLY the student's handwritten/marked ink "
                            f"from question Q{qid} (an MCQ). Printed text has been removed.\n\n"
                            "The student selected their answer by ONE of these methods:\n"
                            "  - Writing a letter (A, B, C, or D)\n"
                            "  - Circling a letter or option\n"
                            "  - Ticking/crossing a letter\n\n"
                            "Use visual reasoning to identify which letter (A/B/C/D) is marked.\n"
                            "Return ONLY valid JSON: "
                            '{"answer": "A"}\n'
                            'If nothing is marked, return: {"answer": ""}'
                        )
                    else:
                        ocr_p = (
                            f"This image shows ONLY the student's handwritten response "
                            f"for question Q{qid} (a written/subjective question). "
                            "Printed text has been removed.\n\n"
                            "Read ALL visible handwritten text in this image.\n"
                            "Return ONLY valid JSON: "
                            '{"answer": "the full handwritten text here"}\n'
                    ...
                    # Fix 3: debug logging of raw GPT output
                    log.info(f"[DIFF-OCR] Q{qid} raw GPT output: {raw_content}")

                    val = json.loads(raw_content).get("answer", "").strip()
                    if val:
                        if qtype == "objective":
                            # Fix 5: extract letter from any length string
                            m = re.search(r"\b([A-D])\b", val, re.IGNORECASE)
                            if m:
                                ...
                                letters = re.findall(r"[A-Da-d]", val)
                                if letters:
                                    page_answers[qid] = letters[0].upper()
                        else:
                            page_answers[qid] = val
                        log.info(f"[DIFF-OCR] Q{qid} ({qtype}): '{page_answers.get(qid, 'no letter found')}'")
                return page_answers

            else:
                # ── Enhanced vision path (no blank reference) ─────────────────
                # Fix 2: apply full preprocessing pipeline
                enhanced = _preprocess_image_for_ocr(img)
                buf = _io.BytesIO()
                enhanced.save(buf, format="PNG")
                img_b64 = base64.b64encode(buf.getvalue()).decode()

                # Fix 1: visual-reasoning prompt
                ...
                # Fix 3: debug logging
                log.info(f"[VISION-OCR] page {page_idx} raw GPT output: {raw}")
...
def _extract_answers(path: str, exam_questions: list = None, exam: dict = None):
    ...
    if ext == ".pdf":
        ...
        try:
            ...
            # Fix 1: use visual-reasoning prompt
            prompt = _build_ocr_prompt(exam_questions)
    ...
    if ext in {".png", ".jpg", ".jpeg", ".webp"}:
        ...
        try:
            ...
            # Fix 2: preprocess image before sending to GPT
            raw_img = _PIL.open(path)
            preprocessed = _preprocess_image_for_ocr(raw_img)
            buf = _io.BytesIO()
